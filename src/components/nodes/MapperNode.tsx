...
import type { MapperNodeData } from "@/types/graph"
  ...
  return (
    <div className="w-72 rounded-lg border border-violet-500/40 bg-card p-3 shadow-sm">
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">Rules: {args.mappings.length}</span>
      </div>
      <p className="text-sm font-semibold text-foreground">{data.label}</p>
      <p className="mt-1 text-xs text-muted-foreground">Maps upstream payload to target output JSON</p>
