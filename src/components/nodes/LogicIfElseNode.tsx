...
export function LogicIfElseNode({ data }: NodeProps<NodeData>) {
  return (
    <div className="w-72 rounded-lg border border-amber-500/40 bg-card p-3 shadow-sm">
      ...
      <p className="text-sm font-semibold text-foreground">{data.logic.leftPath}</p>
      <p className="mt-1 text-xs text-muted-foreground">Compare against: {data.logic.rightValue || "<empty>"}</p>
